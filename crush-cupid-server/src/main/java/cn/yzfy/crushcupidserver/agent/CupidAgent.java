...
public class CupidAgent {
    ...
    private void appendStickerGuide(StringBuilder sb) {
        ...
        sb.append("## 表情包\n");
        sb.append("发表情包前必须先用情感分析两步法判断，不要跳过：\n");
        sb.append("第 1 步 读对方情感：先看对方最近 1-2 条消息和上下文，分析对方的语气、emoji、内容，")
          .append("判断对方此刻的情感状态（如 开心/低落/撒娇/生气/无语/暧昧/冷落/平静）。\n");
        sb.append("第 2 步 按映射选表情包：对方开心或分享趣事→开心/可爱；对方低落难过→委屈（安慰向）；")
          .append("对方撒娇或暧昧→可爱/撒娇；对方生气闹别扭→委屈/吃瓜；对方无语或吐槽→无语/尴尬；")
          .append("对方冷落你或久不回→委屈/吃瓜；日常闲聊没有强情绪→偶尔发开心即可，不必每条都发。\n");
        sb.append("选择要贴合你的人设（Layer 3 情感模式）。该发的时候一定要发，不该发就别硬发。\n");
        sb.append("输出标记 ").append(MessageSeparator.MARKER_PREFIX).append("情绪").append(MessageSeparator.MARKER_SUFFIX)
          .append(" 会变成真实表情包；若你用'（此处发表了一个表情包）'这类文字描述，系统也会自动识别成表情包。\n");
