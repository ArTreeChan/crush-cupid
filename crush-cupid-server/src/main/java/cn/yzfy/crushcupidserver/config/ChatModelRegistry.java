...
public class ChatModelRegistry {
    ...
    public synchronized void reload() {
        ...
        //    只注册 type=chat 或 type 为空（兼容旧数据）的供应商，type=voice 的由 VoiceProviderRegistry 管理
        List<AiProvider> dbProviders = aiProviderService.lambdaQuery()
                .and(w -> w.eq(AiProvider::getType, "chat").or().isNull(AiProvider::getType))
                .list();
