...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        ...
        // 对话图片：磁盘上传目录（/api/uploads/** -> file:...），历史回显用。
        ...
        String location = "file:" + FileUtil.getAbsolutePath(uploadDir).replace('\\', '/');
        if (!location.endsWith("/")) {
            location += "/";
        }
        log.info("注册静态资源映射：handler={} -> location={} (dir={})", handler, location, uploadDir);
